...
class Input(Node):
    def __init__(self):
        ...
        self.rover_sub = self.create_subscription(String, '/command', self.input_callback, 10, callback_group=self.callback_group)
...
class DronePositionControl(DroneObject):
    ...
    def pid(self, error_x, error_y, error_z):
        vel_x = self.kp * error_x + self.kd * ((error_x - self.error_prev_x)/self.dt) + self.ki * error_x * self.dt
        vel_y = self.kp * error_y + self.kd * ((error_y - self.error_prev_y)/self.dt) + self.ki * error_y * self.dt
        vel_z = self.kp * error_z + self.kd * ((error_z - self.error_prev_z)/self.dt) + self.ki * error_z * self.dt

        self.error_prev_x = error_x
        self.error_prev_y = error_y
        self.error_prev_z = error_z
